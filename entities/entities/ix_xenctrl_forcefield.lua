...
ENT.Type = "anim"
ENT.PrintName = "Infestation Control Forcefield"
ENT.Category = "HL2 RP"
ENT.Spawnable = true
ENT.AdminOnly = true
...
if (SERVER) then
	...
	function ENT:Initialize()
		self:SetModel("models/hlvr/combine_hazardprops/combinehazardprops_forcefield_left.mdl")
		...
		self.dummy:SetModel("models/hlvr/combine_hazardprops/combinehazardprops_forcefield_right.mdl")
	...
	local MODES = {
		...
		},
		{
			function(client)
				...
				if (character and character:GetInventory() and !character:GetInventory():HasItem("worker_card")) then
					return true
				else
					return false
				end
			end,
			"Only allow with valid Worker ID."
		},
		{
			function(client)
				return true
